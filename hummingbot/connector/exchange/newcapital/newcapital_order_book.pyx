...
from . import newcapital_utils

_bob_logger = None


cdef class NewcapitalOrderBook(OrderBook):
    @classmethod
    ...
    def snapshot_message_from_exchange(cls,
        ...
        return OrderBookMessage(OrderBookMessageType.SNAPSHOT, {
            ...
            "update_id": int(float(msg.get("lastUpdateId", f"{timestamp}"))),
            "bids": msg["bids"],
            "asks": msg["asks"]
        }, timestamp=timestamp)
    ...
    def diff_message_from_db(cls, record: RowProxy, metadata: Optional[Dict] = None) -> OrderBookMessage:
        msg = ujson.loads(record["json"])  # New Capital json in DB is TEXT
        if metadata:
            msg.update(metadata)
        return OrderBookMessage(OrderBookMessageType.DIFF, {
            "trading_pair": newcapital_utils.convert_from_exchange_trading_pair(msg["s"]),
            "update_id": msg["u"],
            "bids": msg["b"],
            "asks": msg["a"]
        }, timestamp=record["timestamp"] * 1e-3)

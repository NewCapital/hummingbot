...
class NewcapitalTime:
    NEWCAPITAL_TIME_API = "https://api.new.capital/v1/exchangeInfo"
    NaN = float("nan")
    _bt_logger = None
    ...
    @classmethod
    def get_instance(cls) -> "NewcapitalTime":
        if cls._bt_shared_instance is None:
            cls._bt_shared_instance = NewcapitalTime()
    ...
    async def update_server_time_offset(self):
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(self.NEWCAPITAL_TIME_API) as resp:
                    resp_data: Dict[str, float] = await resp.json()
                    newcapital_server_time_ms: float = float(resp_data["serverTime"]) * 1e3
                    local_after_ms: float = time.perf_counter() * 1e3
            local_server_time_pre_image_ms: float = (local_before_ms + local_after_ms) / 2.0
            time_offset_ms: float = newcapital_server_time_ms - local_server_time_pre_image_ms
            self.add_time_offset_ms_sample(time_offset_ms)
            self._last_update_local_time = time.perf_counter()
        except asyncio.CancelledError:
            raise
        except Exception:
            self.logger().network("Error getting New Capital server time.", exc_info=True,
                                  app_warning_msg="Could not refresh New Capital server time. "
